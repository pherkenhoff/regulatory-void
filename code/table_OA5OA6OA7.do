...
*********************************************************************************
***TABLES OA.5, OA.6 and OA.7: MAIN SPECIFICATIONS WITH TOTAL SALES AND PAYROLL***
*********************************************************************************

	capture drop sample*
	capture est drop _all
	
	capture erase ".\results\table_OA5.csv"
	capture erase ".\results\table_OA6.csv"
	capture erase ".\results\table_OA7.csv"
	...
	**TOTAL SALES**
	...
	foreach n in TS  {
		...
		esttab regr_TS* using ".\results\table_OA5.csv", replace scsv ///
					cells(b(fmt(a) star) se(par(( )) fmt(a))) order(waste* c.waste_int_ln_ind_sales#*) drop(0.sigma_median_dummy) /// 
					...
					mlabels(- - EPSI EPSI EPSI EPSI HLT HLT HLT HLT, lhs(Intensity Definition: Total Sales, Environmental Index:)) ///
		...
		**PAYROLL**
		...
		esttab regr_PR* using ".\results\table_OA6.csv", replace scsv ///
					cells(b(fmt(a) star) se(par(( )) fmt(a))) order(waste* c.waste_int_ln_payroll#*) drop(0.sigma_median_dummy) /// 
					stats(N_clust r2_a N, fmt(a) labels("IO2007 Industry Clusters" "Adj. R2" "Observations")) /// 
					obslast starlevels(* 0.10 ** 0.05 *** 0.01) collabels(none) title("Dependent Variable: Intrafirm Import Share") ///
					mlabels(- - EPSI EPSI EPSI EPSI HLT HLT HLT HLT, lhs(Intensity Definition: Payroll, Environmental Index:)) ///
					label interact(" X ") noomit indicate(`r(indicate_fe)')					
		
		
		esttab marg* using ".\results\table_OA7.csv", replace scsv ///
					cells(b(fmt(a) star) se(par(( )) fmt(a))) ///
					noobs starlevels(* 0.10 ** 0.05 *** 0.01) collabels(none) label ///
					mlabels(EPSI EPSI EPSI HLT HLT HLT EPSI EPSI EPSI HLT HLT HLT, lhs(ENVI:)) coeflabels(waste_int_ln_totalcost_2 "Percentile")
