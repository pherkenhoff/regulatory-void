...
********************************************************
***TABLES OA.3 and OA.4: AGGREGATED CAPITAL INTENSITY***		
********************************************************

	capture drop sample*
	capture est drop _all

	capture erase ".\results\table_OA3.csv"
	capture erase ".\results\table_OA4.csv"
	
	foreach n in TC {
			reghdfe rel_share ${AYcontrols_agg_`n'}, absorb(year_iso3code) vce(cluster i.IO2007_num)
				est store regr_`n'_noECSP
				estfe regr_`n'_noECSP, labels(year_iso3code "Country-Year FE")
			reghdfe rel_share ${ECSP_`n'} ${AYcontrols_agg_`n'}, absorb(year_iso3code) vce(cluster i.IO2007_num)
				est store regr_`n'_ECSP
				estfe regr_`n'_ECSP, labels(year_iso3code "Country-Year FE")
		
		foreach m in EPSI HLT EPI {
			
			reghdfe rel_share ${ECSP_`n'}  c.${ECSP_`n'}#c.${`m'} ${AYcontrols_agg_`n'}, absorb(year_iso3code) vce(cluster i.IO2007_num)
			...
			reghdfe rel_share ${ECSP_`n'}  c.${ECSP_`n'}#c.${`m'} ${AYcontrols_agg_`n'} ${HO_interact_nocou_`n'} ${contract_interact_nocou}, absorb(year_iso3code) vce(cluster i.IO2007_num)
			...
			reghdfe rel_share ${ECSP_`n'}  c.${ECSP_`n'}#c.${`m'} ${AYcontrols_agg_`n'} ${disp} ${Antras_Chor} ${HO_interact_nocou_`n'} ${contract_interact_nocou} ${credit_interact_nocou} ${labormarket_interact_nocou} ${interm_interact}, absorb(year_iso3code) vce(cluster i.IO2007_num)
			...
			reghdfe rel_share  c.${ECSP_`n'}#c.${`m'} ${HO_interact_nocou_`n'} ${credit_interact_nocou} ${labormarket_interact_nocou} ${contract_interact_nocou} ${interm_interact}, absorb(year_iso3code year_ind)  vce(cluster i.IO2007_num)
		...
		esttab regr* using ".\results\table_OA3.csv", replace scsv ///
					...
					mlabels(- - EPSI EPSI EPSI EPSI HLT HLT HLT HLT EPI EPI EPI EPI, lhs(Intensity Definition: Total Cost, Environmental Index:)) ///
					label interact(" X ") noomit indicate(`r(indicate_fe)') 
		
		esttab marg* using ".\results\table_OA4.csv", replace scsv ///
